...
contract ERC20ExchangeMarketMaker {
    ...
    // Exchange structure
    struct Exchange {
        Token[2] tokens; // token pair in exchange
    ...
    /// @param tokenIndex `0` or `1` to refer to either first or second entry in the token pair associated with exchange
    ///        as determined by `getExchange`
    ...
    function getExchange(bytes32 exchangeIdentifier)
        public
        constant
        returns (Token[2] tokens, uint[2] supplies)
    {
        Exchange ex = exchanges[exchangeIdentifier];
        tokens = ex.tokens;
        ...
        returns (uint[2])
    {
        Exchange ex = exchanges[exchangeIdentifier];
        uint param = 0;
        if (now > ex.lastUpdateTimestamp + PRICE_LOCK_DURATION) {
            param = now - ex.lastUpdateTimestamp - PRICE_LOCK_DURATION;
            if (param > PRICE_RAMP_DURATION)
    ...
    /// @dev Will price `tokens[tokenIndex]` to keep `supplies[0] * supplies[1]` the same after sending `amount` of
    ///      `tokens[tokenIndex]` to `msg.sender` and receiving the calculated price of `tokens[1-tokenIndex]` from `msg.sender`
    /// @param exchangeIdentifier The ID of the exchange
    /// @param tokenIndex Index of the token to be bought from the exchange
    /// @param amount Amount of `tokens[tokenIndex]` to buy from exchange
    ...
    /// @return Exchange ID
    function calcExchangeIdentifier(Token[2] tokens)
        public
        constant
        returns (bytes32)
    {
        return keccak256(address(tokens[0])) ^ keccak256(address(tokens[1]));
    }
}
