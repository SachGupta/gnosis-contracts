...
class TestContract(AbstractTestContract):
    """
    run test with python -m unittest contracts.tests.do.test_bidding_ring
    """
    ...
    def test(self):
        ...
        self.bidding_ring = self.s.abi_contract(
            self.pp.process(self.DO_DIR + 'BiddingRing.sol', add_dev_code=True,
                            contract_dir=self.contract_dir),
            language='solidity',
            constructor_parameters=(self.dutch_auction.address, max_price)
        )
        # Collect bids
        bidder_1 = 1
